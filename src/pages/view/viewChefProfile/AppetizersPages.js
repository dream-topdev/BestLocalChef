...
import { Text, FlatList, View, Image, StyleSheet, ScrollView, Modal, TouchableOpacity } from "react-native";
...
const styles = StyleSheet.create({
    ...
    mealsList:{
        width:120, borderColor: '#fff', justifyContent: 'space-between', marginTop: 10, marginRight:10
    },
    mealImage:{
        width:120, height:120, borderRadius:5, 
    },
    mealsText:{
...
class AppetizersPages extends Component {

    constructor(props){
        super(props);
        this.state = {
            visibleApplizerModal: false,
            item:[]
        }
    }

    setModalVisible(item) {
        this.setState({visibleApplizerModal: true});
    ...
    _renderItems() {
        ...
        return coordinateData.menus_appetizers.map((ite) => {
            return (
                <TouchableOpacity style={styles.mealsList} onPress={() => this.setModalVisible(ite)}>
    ...
    viewMealModel(){
        ...
        return (
        <Modal animationType='slide' visible={this.state.visibleApplizerModal}  transparent={true}>
          <ScrollView style={{ backgroundColor: 'rgba(50, 50, 50, .9)', width: '100%', height: '100%' }}>
                <View style={styles.modelStyle}>

                  <TouchableOpacity 
                    onPress={() => this.setState({ visibleApplizerModal: false })}
...
export default connect(mapStateToProps, mapDispatchToProps)(AppetizersPages);