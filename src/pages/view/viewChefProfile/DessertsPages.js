...
class DessertsPages extends Component {

    constructor(props){
        super(props);
        this.state = {
            visibleDessertModal: false,
            item:[]
        }
    }

    setModalVisible(item) {
        this.setState({visibleDessertModal: true});
        this.setState({item: item});
    }

    _renderItems() {
        const {propsData: {coordinateData}} = this.props;
        return coordinateData.menus_desserts.map((ite) => {
            return (
    ...
    viewMealModel(){
        ...
        return (
        <Modal animationType='slide' visible={this.state.visibleDessertModal}  transparent={true}>
          <ScrollView style={{ backgroundColor: 'rgba(50, 50, 50, .9)', width: '100%', height: '100%' }}>
                <View style={styles.modelStyle}>

                  <TouchableOpacity 
                    onPress={() => this.setState({ visibleDessertModal: false })}
...
export default connect(mapStateToProps, mapDispatchToProps)(DessertsPages);